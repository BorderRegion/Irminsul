...
class SqlLedgerStore(private val dataSource: DataSource?) : LedgerStore {
    ...
    override fun ensureTables() = transaction {
    ...
    override suspend fun setupCache() {
        execute {
    ...
    override suspend fun selectRollback(params: ActionSearchParams): List<ActionType> = execute {
    ...
    override suspend fun selectRestore(params: ActionSearchParams): List<ActionType> = execute {
        val query = buildQuery()
    ...
    override suspend fun previewActions(
    ...
    override suspend fun logActionBatch(actions: List<ActionType>) {
        execute {
            insertActions(actions)
        }
    }

    override suspend fun registerWorld(identifier: Identifier) =
        execute {
            insertWorld(identifier)
        }

    override suspend fun registerActionType(id: String) =
        execute {
            insertActionType(id)
        }

    override suspend fun logPlayer(uuid: UUID, name: String) =
        execute {
            insertOrUpdatePlayer(uuid, name)
        }

    override suspend fun insertIdentifiers(identifiers: Collection<Identifier>) =
        execute {
    ...
    override fun getKnownSources(): Set<String> =
